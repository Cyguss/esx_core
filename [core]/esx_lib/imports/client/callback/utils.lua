...
---@return string
function Utils.nextRequestKey(state, eventName)
    local key

    repeat
        state.requestId += 1
        key = ("%s:%s:%s:%s"):format(eventName, GetGameTimer(), state.requestId, math.random(100000, 999999))
...
---@return table
function Utils.createPending(name, requestPromise, callbackFn)
    local pending = {
        name = name,
    }
